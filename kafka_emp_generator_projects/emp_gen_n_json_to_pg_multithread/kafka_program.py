...
from for_program.producer import pro
from for_program.consumer import con
from kafka_logging.kafka_logger import kafka_log
from json_utilities.to_postgres import to_pg
# --------------------------------------------------------------
...
class KafkaProgram:
    ...
    def kafka_program(self):
        try:
            ...
            # Select JSON file to load to Postgres
            json_file_number = int(input('Enter JSON File Number: '))
            to_pg.json_to_postgres(f'json_utilities\\json_files\\thread-{json_file_number}.json')
